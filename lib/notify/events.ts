...
/** booking completed → invite the client to review (token link). */
export async function notifyBookingCompleted(bookingId: string): Promise<void> {
  try {
    const c = await loadContacts(bookingId);
    if (!c) return;
    const base = process.env.NEXT_PUBLIC_SITE_URL || "http://localhost:3000";
    const url = `${base}/otziv/${signReviewToken(bookingId)}`;
    const t = completedToClient(c.info, url);
    await sendNotifications([...email(c.clientEmail, t), ...sms(c.clientPhone, t)]);
  } catch (e) {
    console.error("[notify] notifyBookingCompleted failed:", e);
  }
}
